...
public class BookingServiceImpl implements BookingService {
	...
	private ConsumerOrderEntity createConsumerOrderEntity(HttpServletRequest request,
		...
		Optional<ProducerOrderEntity> optionalProducerOrderEntity = producerOrderRepository
				.findById(consumerOrder.getProducerOrderId());
		if (optionalProducerOrderEntity != null && optionalProducerOrderEntity.get() != null) {
			ProducerOrderEntity producerOrderEntity = optionalProducerOrderEntity.get();
	...
	public CommonResponseDTO fetchAllProducerOrders() {
		...
		if (!prodEntities.isEmpty()) {
			...
			if (dataResponseDTO.getSuccess()) {
				...
				Map<String, ProducerOrderResponseDto> responseMap = getResponseMap(
	...
	@SuppressWarnings("unchecked")
	private Map<String, ProducerOrderResponseDto> getResponseMap(Map<String, Object> userAddressMap,
			List<ProducerOrderEntity> prodEntities) {
		Map<String, ProducerOrderResponseDto> producerOrderResponseMap = new HashMap<String, ProducerOrderResponseDto>();
		if (!userAddressMap.isEmpty() && !prodEntities.isEmpty()) {
			for (ProducerOrderEntity prodEntity : prodEntities) {
				if (prodEntity.getEmail() != null) {
					...
					producerOrderResponseDto.setImageThumbnailUrls(getThumbnaiImageUrls(prodEntity.getItemList()));
					...
					producerOrderResponseMap.put("ProducerOrder", producerOrderResponseDto);
				}
			}
		}
	...
	private List<String> getThumbnaiImageUrls(Set<ItemEntity> itemList) {
		List<String> imageUrls = new ArrayList<String>();
		for (ItemEntity itemEntity : itemList) {
	...
	public CommonResponseDTO fetchSingleItem(Long producerOrderId) {
		...
		if (!Util.isLongValueEmpty(producerOrderId)) {
			Optional<ProducerOrderEntity> optionalProdEntity = producerOrderRepository.findById(producerOrderId);
			if (optionalProdEntity != null) {
				ProducerOrderEntity prodEntity = optionalProdEntity.get();
				if (prodEntity != null) {
